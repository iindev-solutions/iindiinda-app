	...
	<div class="min-h-screen px-4 py-6 pb-8">
		<div class="mx-auto max-w-[480px]">
			<div class="flex flex-col items-center justify-center py-12">
				...
				<UCard class="mb-6 w-full rounded-2xl border-gray-800 bg-level-1">
					<div class="text-center">
						<div class="mb-3 text-sm text-gray-400">{{ t('ayan.complete.ratingPrompt') }}</div>
			...
			<UCard class="mt-8 rounded-2xl border-gray-800 bg-level-1">
				<div class="space-y-4">
					<h3 class="text-center font-medium text-white">{{ t('ayan.complete.tripDetails') }}</h3>
					<div class="grid grid-cols-2 gap-4">
						<div class="text-center">
							<div class="mb-1 text-xs text-gray-500">{{ t('ayan.complete.date') }}</div>
							<div class="text-sm text-white">{{ new Date().toLocaleDateString('ru-RU') }}</div>
						</div>
						<div class="text-center">
							<div class="mb-1 text-xs text-gray-500">{{ t('ayan.complete.time') }}</div>
